...
extension NSImage {
    ...
    func imageToJPEGData(compress: Double? = nil) -> Data? {
        guard let imageTiffData = self.tiffRepresentation,
              let imageRep = NSBitmapImageRep(data: imageTiffData) else {return nil}
        
        var imageProps: [NSBitmapImageRep.PropertyKey : Any] = [:]
        
        //Добавляем компрессию
        if let compress = compress {
            imageProps[NSBitmapImageRep.PropertyKey.compressionFactor] = compress
        }
